module Celestine::Test::Mineshift
  ...
  def self.make_new_img
    @@seed &+= 1
    File.open("./bin" + make_img_path(@@seed), "w+") { |f| f.puts self.make(@@seed) }
  end
  ...
  def self.make(make_seed)
    @@perlin = PerlinNoise.new(make_seed.to_i)
      ...
      MAX_LEVELS.times do |level|
        mask = ctx.mask do |mask|
          ...
          ctx.group do |group|
            ...
            # TODO: Remove debug bbs
            # bridge_bounding_boxes.each do |bb|
            #   bb.id = "bounding-box"
            #   group << bb
            # end
